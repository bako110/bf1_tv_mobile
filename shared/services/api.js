...
export async function getLiveStreamUrl() {
  return `${http.getBaseUrl()}/livestream/stream-proxy`;
}
...
export async function getCarousel() {
  return http.get('/carousel').catch(() => []);
}
...
export async function getMovies(skip = 0, limit = 20) {
  ...
  return { items: res.items || res.movies || (Array.isArray(res) ? res : []), total: res.total || 0, skip, limit };
}
...
export async function getSports(skip = 0, limit = 20) {
  ...
  return { items: res.items || res.sports || (Array.isArray(res) ? res : []), total: res.total || 0, skip, limit };
}
...
export async function getPrograms() {
  return http.get('/programs').catch(() => []);
}
...
export async function getShowById(id, type) {
  switch (type) {
    ...
    case 'series':           return http.get(`/series/${id}`);
...
export async function getFavorites() {
  return http.get('/favorites').catch(() => []);
}
