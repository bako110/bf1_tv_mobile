...
function statusLabel(status) {
  const map = { ongoing: 'En cours', completed: 'Terminée', cancelled: 'Annulée', hiatus: 'En pause' };
  return map[status] || status || '';
}

function buildCard(serie) {
  const id = serie.id || serie._id;
  const title = serie.title || 'Sans titre';
  const img = serie.image_url || serie.banner_url || serie.poster || '';
  const genres = Array.isArray(serie.genre) ? serie.genre.slice(0, 2) : [];
  const seasons = serie.total_seasons || 0;
  const year = serie.release_year || (serie.created_at ? new Date(serie.created_at).getFullYear() : null);
  const isPremium = serie.is_premium;
  const status = statusLabel(serie.status);

  return `
    <div onclick="window.location.hash='#/series-detail/${esc(id)}'"
      ...
      ${img
        ...
        : `<div style="width:100%;height:100%;background:#1a1a1a;display:flex;align-items:center;justify-content:center;">
             <i class="bi bi-collection-play" style="font-size:40px;color:#333;"></i>
           </div>`}

      <!-- Gradient -->
      <div style="position:absolute;inset:0;background:linear-gradient(transparent 40%,rgba(0,0,0,0.97) 100%);"></div>

      <!-- Premium -->
      ${isPremium ? `<div style="position:absolute;top:8px;left:8px;">
        <span style="background:#FF6F00;color:#fff;font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;">PREMIUM</span>
      </div>` : ''}

      <!-- Seasons badge -->
      ${seasons > 0 ? `<div style="position:absolute;top:8px;right:8px;">
  ...
  try {
    ...
    if (!series.length) {
      container.innerHTML = `
        <div class="text-center" style="padding:60px 20px;">
          <i class="bi bi-collection-play" style="font-size:3rem;color:#333;"></i>
          <p style="color:#555;margin-top:12px;font-size:14px;">Aucune série disponible pour le moment</p>
        </div>`;
      return;
    }
    ...
    container.innerHTML = `
      <div style="padding:12px;display:grid;grid-template-columns:1fr 1fr;gap:10px;padding-bottom:70px;">
        ${series.map(buildCard).join('')}
      </div>`;

  } catch (err) {
    console.error('Erreur loadSeries:', err);
    container.innerHTML = `
      <div class="text-center" style="padding:60px 20px;">
        ...
        <button onclick="window.location.hash='#/series'"
