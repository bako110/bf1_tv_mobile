...
export async function loadDivertissement() {
  ...
  const pageTitle = document.querySelector('#divertissement-page .page-title');
  if (pageTitle && catFilter) pageTitle.textContent = catFilter;

  injectSortBar('divertissement-page', (order) => {
    currentSort = order;
    renderList(listEl);
  ...
  try {
    const data = await api.getDivertissement(0, LIMIT, catFilter).catch(() => ({ items: [], total: 0 }));
    allItems = data.items || [];
...
function attachInfiniteScroll(listEl) {
  setupInfiniteScroll({
    listEl, sentinelId: 'divertissement-sentinel',
    fetchFn: (skip, limit) => api.getDivertissement(skip, limit),
    renderCard: (item) => currentMode === 'grid' ? buildGridCard(item) : buildListCard(item),
    getSkip: () => currentSkip, getTotal: () => currentTotal,
    onNewItems: (items, total) => { allItems = [...allItems, ...items]; currentSkip += items.length; if (total) currentTotal = total; },
...
function renderList(container) {
  ...
  const sorted = applySortFilter(allItems, currentSort, 'created_at', 'published_at');
  ...
  if (api.isAuthenticated()) {
    allItems.forEach((item, index) => {
      ...
      if (contentId) {
        api.checkFavorite('divertissement', contentId).then(isFavorited => {
...
function buildGridCard(item, index = 0) {
  ...
  const addButton = `
    <div onclick="event.stopPropagation();event.preventDefault();window.toggleFavorite('divertissement','${id}',this)" style="position:absolute;bottom:8px;right:8px;width:36px;height:36px;background:rgba(0,0,0,0.7);backdrop-filter:blur(10px);border-radius:8px;display:flex;align-items:center;justify-content:center;color:white;font-size:18px;z-index:3;border:1px solid rgba(255,255,255,0.15);cursor:pointer;">
      <i class="bi bi-plus-lg"></i>
    </div>
  `;

  return `
    <a href="#/show/divertissement/${id}" class="bf1-content-card" style="--card-index:${index};text-decoration:none;display:block;width:100%;">
...
function buildListCard(item, index = 0) {
  ...
  return `
    <a href="#/show/divertissement/${id}" class="bf1-list-card-link" style="--card-index:${index};text-decoration:none;display:block;animation:cardFadeIn 0.55s cubic-bezier(0.22,1,0.36,1) both;animation-delay:${index * 70}ms;opacity:0;">
