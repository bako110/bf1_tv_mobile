...
function renderList(container) {
  ...
  container.innerHTML = `<div class="bf1-cards-wrapper px-3 pt-2 pb-3" style="${wrapStyle}">${allShows.map(isGrid ? buildGridCard : buildListCard).join('')}</div>`;
}
  ...
  return `
    <div class="d-flex mb-3" style="background:#1a1a1a;border-radius:10px;overflow:hidden;cursor:pointer;" onclick="window.location.hash='#/show/jtandmag/${item.id||item._id}'">
      ...
      <div class="d-flex flex-column justify-content-between p-2" style="flex:1;overflow:hidden;">
        <p class="mb-1 fw-semibold" style="font-size:13px;color:var(--description-list-color,#fff);overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">${esc(title)}</p>
