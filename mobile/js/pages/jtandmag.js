...
export async function loadJTandMag() {
  ...
  try {
    ...
    setupInfiniteScroll({
      ...
      fetchFn: (skip, limit) => api.getJTandMag(skip, limit),
    ...
    listEl.innerHTML = emptyState('bi-camera-video', 'Impossible de charger les Journaux');
  }
}
...
function renderList(container) {
  ...
  if (api.isAuthenticated()) {
    allShows.forEach((item, index) => {
      ...
      if (contentId) {
        api.checkFavorite('jtandmag', contentId).then(isFavorited => {
...
function buildGridCard(item, index = 0) {
  ...
  const addButton = `
    <div onclick="event.stopPropagation();event.preventDefault();window.toggleFavorite('jtandmag','${id}',this)" style="position:absolute;bottom:8px;right:8px;width:36px;height:36px;background:rgba(0,0,0,0.7);backdrop-filter:blur(10px);border-radius:8px;display:flex;align-items:center;justify-content:center;color:white;font-size:18px;z-index:3;border:1px solid rgba(255,255,255,0.15);cursor:pointer;">
      <i class="bi bi-plus-lg"></i>
    </div>
  `;

  return `
    <a href="#/show/jtandmag/${id}" class="bf1-content-card" style="--card-index:${index};text-decoration:none;display:block;width:100%;">
