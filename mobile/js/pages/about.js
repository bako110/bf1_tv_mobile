...
function renderAbout(container) {
  container.innerHTML = `
    ...
    <div style="background:linear-gradient(160deg,rgba(226,62,62,0.12) 0%,var(--surface,#111) 60%);
                padding:40px 20px 28px;border-bottom:1px solid var(--divider,#1e1e1e);">
      <div class="d-flex flex-column align-items-center text-center">
        ...
        <h2 class="fw-bold mb-1" style="font-size:22px;color:var(--text,#fff);">BF1 TV</h2>
        <p style="color:var(--text-2,#A0A0A0);font-size:13px;font-style:italic;margin:0;">
          La chaîne au cœur de nos défis
        </p>
        <span class="badge mt-3" style="background:rgba(226,62,62,0.15);color:#E23E3E;
              border:1px solid rgba(226,62,62,0.27);font-size:12px;letter-spacing:.5px;">
          Version 1.0.0
        </span>
      </div>
    </div>
    ...
    <div class="px-3 pt-3">
      ...
      <div class="mb-4 p-3 rounded" style="background:var(--surface,#1a1a1a);border:1px solid var(--divider,#2a2a2a);">
        <div class="d-flex align-items-center gap-2 mb-2">
          ...
          <span class="fw-bold" style="font-size:15px;color:var(--text,#fff);">Notre Mission</span>
        </div>
        <p style="color:var(--text-2,#C0C0C0);font-size:13px;line-height:1.7;margin:0;">
      ...
      <!-- Légal -->
      <div class="mb-2 p-3 rounded" style="background:var(--bg,#111);border:1px solid var(--divider,#222);">
        <div class="d-flex justify-content-between align-items-center py-1">
          <span style="font-size:13px;color:var(--text-2,#A0A0A0);">Mentions légales</span>
          <i class="bi bi-chevron-right" style="color:var(--text-3,#555);font-size:13px;"></i>
        </div>
        <div class="d-flex justify-content-between align-items-center py-1" style="border-top:1px solid var(--divider,#222);">
          <span style="font-size:13px;color:var(--text-2,#A0A0A0);">Politique de confidentialité</span>
          <i class="bi bi-chevron-right" style="color:var(--text-3,#555);font-size:13px;"></i>
        </div>
        <div class="d-flex justify-content-between align-items-center py-1" style="border-top:1px solid var(--divider,#222);">
          <span style="font-size:13px;color:var(--text-2,#A0A0A0);">Conditions d'utilisation</span>
          <i class="bi bi-chevron-right" style="color:var(--text-3,#555);font-size:13px;"></i>
        </div>
      </div>

      <p class="text-center mt-3 mb-2" style="color:var(--text-3,#555);font-size:11px;">
...
function feature(icon, title, desc) {
  return `
    <div class="d-flex align-items-center gap-3 p-3 rounded"
         style="background:var(--surface,#111);border:1px solid var(--divider,#2a2a2a);">
      ...
      <div>
        <div style="font-size:14px;font-weight:600;color:var(--text,#fff);">${title}</div>
        <div style="font-size:12px;color:var(--text-2,#A0A0A0);">${desc}</div>
      </div>
    </div>`;
}
...
function socialLink(icon, color, platform, handle, url) {
  return `
    <a href="${url}" target="_blank" rel="noopener"
       ...
       onmouseover="this.style.background='rgba(226,62,62,0.06)'"
       onmouseout="this.style.background=''">
      <i class="bi ${icon}" style="font-size:22px;color:${color};width:28px;text-align:center;"></i>
      <div>
        <div style="font-size:13px;font-weight:500;color:var(--text,#fff);">${platform}</div>
        <div style="font-size:12px;color:var(--text-2,#A0A0A0);">${handle}</div>
      </div>
      <i class="bi bi-arrow-up-right ms-auto" style="color:var(--text-3,#555);font-size:13px;"></i>
    </a>`;
}

function infoRow(icon, label, value) {
  return `
    <div class="d-flex align-items-start gap-3 py-2" style="border-bottom:1px solid var(--divider,#2a2a2a);">
      <i class="bi ${icon}" style="color:#E23E3E;font-size:16px;margin-top:2px;"></i>
      <div>
        <div style="font-size:12px;color:var(--text-3,#A0A0A0);">${label}</div>
        <div style="font-size:13px;color:var(--text,#fff);">${value}</div>
      </div>
    </div>`;
}
