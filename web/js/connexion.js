...
loginFormElement.addEventListener('submit', async (e) => {
  ...
  if (!identifier) {
    showFieldError('loginIdentifier', 'Email ou nom d\'utilisateur requis');
    hasError = true;
  }
...
registerFormElement.addEventListener('submit', async (e) => {
  ...
  if (!username) {
    showFieldError('registerUsername', 'Nom d\'utilisateur requis');
    hasError = true;
  } else if (username.length < 3) {
    showFieldError('registerUsername', 'Le nom d\'utilisateur doit contenir au moins 3 caractères');
    hasError = true;
  }
  
  if (!email) {
    showFieldError('registerEmail', 'Email requis');
    hasError = true;
  } else if (!validateEmail(email)) {
    showFieldError('registerEmail', 'Email invalide');
    hasError = true;
  }
  
  if (!password) {
    showFieldError('registerPassword', 'Mot de passe requis');
    hasError = true;
  ...
  if (!confirmPassword) {
    showFieldError('registerConfirm', 'Confirmation du mot de passe requise');
    hasError = true;
