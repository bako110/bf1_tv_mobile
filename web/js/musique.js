...
export async function loadMusiqueContent() {
  ...
  container.innerHTML = `
    <div class="text-center py-5">
      ...
      <p class="mt-3 text-secondary">Chargement de la musique...</p>
    </div>
  `;

  try {
    const newsData = await api.get();
    musiqueData = Array.isArray(newsData) ? newsData.filter(item => 
      (item.category === 'Musique' || item.edition === 'Musique')
    ) : [];

    if (filterContainer) {
      renderFilters(filterContainer);
    }
    renderMusiqueList(container);

  } catch (error) {
    console.error('❌ Erreur chargement musique:', error);
    container.innerHTML = `
      <div class="text-center py-5">
        <i class="bi bi-music-note-fill" style="font-size:3rem;color:#666;"></i>
        <p class="mt-3 text-secondary">Impossible de charger la musique</p>
        <button class="btn btn-outline-danger btn-sm mt-2" onclick="loadMusiqueContent()">
...
function renderFilters(container) {
  const categories = ['Tous', 'Concerts', 'Clips', 'Afrobeat', 'Coupé-Décalé', 'Traditionnel'];
  container.innerHTML = '';
  ...
  categories.forEach(category => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (mainContainer) renderMusiqueList(mainContainer);
    });
...
function renderMusiqueList(container) {
  let filtered = musiqueData;
  ...
  if (sorted.length > 0) {
    const articlesHTML = pageItems.map((item, index) => buildMusiqueCard(item, startIndex + index)).join('');
    ...
    container.innerHTML = `
      <div class="text-center py-5">
        <i class="bi bi-music-note-fill" style="font-size:3rem;color:#666;"></i>
        <p class="mt-3 text-secondary">Aucun contenu musical disponible</p>
      </div>
    `;
  }
}

function buildMusiqueCard(item, index) {
  const img = item.image_url || item.image || item.thumbnail || '';
  const category = item.category || item.edition || 'Musique';
  const title = item.title || 'Sans titre';
  const description = item.description || item.content || '';
  const author = item.author || 'BF1 Musique';
  ...
  return `
    <div class="news-item anim-up d${(index % 6) + 1}" 
      ...
      <div>
        <span class="news-cat">
          <i class="bi bi-music-note-fill"></i>${escHtml(category)}
        </span>
    ...
    const trendsHTML = trends.map((item) => {
      ...
      return `
        <div class="news-item" style="padding:10px;border-radius:var(--radius-md);background:var(--bg-card);border:1px solid var(--border);cursor:pointer;transition:all var(--transition)"
             onclick="window.location.href='${getNewsDetailUrl(item.title, item.id || item._id)}'">
          <img class="news-thumb" src="${img || 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=120&q=60'}" 
          ...
          <div>
            <span class="news-cat" style="font-size:0.6rem;padding:2px 6px">
              <i class="bi bi-music-note-fill"></i>Musique
            </span>
...
window.loadMusiqueContent = loadMusiqueContent;
