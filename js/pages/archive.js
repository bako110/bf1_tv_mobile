...
function renderList(container) {
  ...
  if (currentMode === 'grid') {
    container.innerHTML = `${banner}<div class="px-3 pt-2 pb-5" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">${allItems.map(buildGridCard).join('')}</div>`;
  } else {
    container.innerHTML = `${banner}<div class="px-3 pt-2 pb-5">${allItems.map(buildListCard).join('')}</div>`;
  }
}
